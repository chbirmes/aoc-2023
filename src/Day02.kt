...
fun main() {
    ...
    data class Game(val id: Int, val cubeSets: List<Map<String, Int>>) {
        ...
        fun minimumSet(): Map<String, Int> = cubeSets.fold(emptyMap()) { x, y ->
            val allColors = x.keys + y.keys
            allColors.associateWith { max(x[it] ?: 0, y[it] ?: 0) }
        }
    }

    fun parseGame(line: String) = line.split(':', ';').let { gameTagAndTurns ->
        val gameId = gameTagAndTurns.first().substringAfter(' ').toInt()
        val cubeSets = gameTagAndTurns.drop(1).map { turn ->
            turn.split(',').associate { countAndColor ->
                countAndColor.trim().split(' ').let {
                    it[1] to it[0].toInt()
                }
            }
        }
    ...
    fun Map<String, Int>.power() = powerColors.map { get(it) ?: 0 }.fold(1) { x, y -> x * y }
